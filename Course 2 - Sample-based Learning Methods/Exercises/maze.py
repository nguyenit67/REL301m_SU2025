...
class PriorityQueue:
    ...
    def add_item(self, item: tuple, priority: float = 0) -> None:
...
class Maze:
    def __init__(self):
        ...
        self.old_obstacles: list[list[int]] | None = None
        self.new_obstacles: list[list[int]] | None = None
    ...
    def extend_maze(self, factor: int) -> "Maze":
        new_maze = Maze()
    ...
    def step(self, state: list, action: int) -> tuple:
        x, y = state
...
def choose_action(state: list, q_value: np.ndarray, maze: Maze, dyna_params: DynaParams) -> int:
...
class TrivialModel:
    ...
    def feed(self, state: list, action: int, next_state: list, reward: float) -> None:
...
class TimeModel:
    ...
    # @rand: an instance of np.random.RandomState for sampling
    def __init__(self, maze: Maze, time_weight: float = 1e-4, rand=np.random):
        self.rand = rand
        self.model = dict()
