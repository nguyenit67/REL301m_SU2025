...
class IHT:
    ...
    def __init__(self, size_val):
        ...
        self.dictionary = {}  # Maps coordinates to indices
    ...
    def get_index(self, obj, read_only=False):
        # Returns unique index for coordinate tuple
        # Handles hash collisions when table is full
        d = self.dictionary
        if obj in d:
            return d[obj]
        elif read_only:
...
def differential_semi_gradient_sarsa(value_function, max_steps):
    ...
    for _ in tqdm(range(max_steps)):
        ...
        value_function.learn(
            current_free_servers, current_priority, current_action, new_free_servers, new_priority, new_action, reward
        )
